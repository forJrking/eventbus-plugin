package com.forjrking.eventbus.lineprovider

import com.forjrking.eventbus.action.ShowUsagesAction
import com.forjrking.eventbus.filter.Filter
import com.forjrking.eventbus.filter.PosterFilter
import com.forjrking.eventbus.filter.ReceiverFilter
import com.forjrking.eventbus.util.*
...
class EventBus3LineMarkerProvider : LineMarkerProvider {
    ...
    override fun getLineMarkerInfo(psiElement: PsiElement): LineMarkerInfo<*>? {
        ...
        return when {
            ...
            PsiUtils.isEventBusReceiver(psiElement) -> {
                ...
                LineMarkerInfo(
                    markElement, markElement.textRange ?: methodElement.textRange,
                    ICON_NAV, { "Show Poster" },
            ...
            psiMethod != null -> {
                if (psiMethod?.fullName in ConfigHelper.postMethodSet) {
                    ...
                    LineMarkerInfo(
                        psiElement, markElement.textRange ?: psiMethod!!.textRange,
                        ICON_BUS, { "Show References" },
    ...
    private fun showPosters(e: MouseEvent, ele: PsiElement) {
        ...
        ShowUsagesAction(PosterFilter(paramType), ShowUsagesAction.TYPE_POST).startFindUsages(
            findEleUsageList.toTypedArray(),
            RelativePoint(e),
    ...
    private fun showReceivers(e: MouseEvent, psiElement: PsiElement) {
        ...
        if (psiElement is KtReferenceExpression) {
            ...
            ShowUsagesAction(
                ...
                subscribeCls,
                RelativePoint(e), PsiEditorUtil.findEditor(psiElement), MAX_USAGES
            )
        }
    }

    private fun showReferences(e: MouseEvent, psiElement: PsiElement) {
        ShowUsagesAction(Filter { true }, -1).startFindUsages(
            psiElement,
            RelativePoint(e), PsiEditorUtil.findEditor(psiElement), MAX_USAGES
        )
